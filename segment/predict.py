...
def run(
        ...
        trk = False,
        save_mask = True, # save masks to *.png
):  
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            mask_path = str(save_dir / p.stem) + ('' if dataset.mode == 'image' else f'_{frame}') # im.png
            ...
            if len(det):
                ...
                #将masks提取出来并保存到png文件中以用于裂缝后续宽度、高度等信息的提取
                if save_mask:
                    skimage.io.imsave(mask_path+ ".png" , skimage.img_as_uint(torch.clamp(torch.sum(masks.cpu(),dim = -1),-1,1)))
...
def parse_opt():
    ...
    parser.add_argument('--save-mask', action='store_false', help='save segment-masks to *.png')
